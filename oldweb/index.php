<?php
    ...
    // Tabelle auf existenz pr�fen & Anzahl der Datens�tze ermitteln
    ...
    // �berpr�fe ob user und passwort ok sind
    // Username und Passwort gesetzt? Wenn ja �berpr�fe auf Konsistenz. Wenn nicht Clean setze user + pass leeren string. wird unten wichtig
...
<?php
    ...
    // Assoziatives Array erstellen, das als Schl�ssel die tabellennamen und als Inhalt das Spaltentemplate hat
    // var_dump($HEADER);
    ...
    if (isset($_GET['limit'])) { if (!(in_array($_GET['limit'], $HEADER))) { unset($_GET['limit']); } } // Wenn �bergebene Spalten-Einschr�nkung kein Spaltenname ist, dann l�sche Einschr�nkung

    if (isset($_GET['ordnung'])) {  // Wenn �bergebene Spaltenordnung kein Spaltenname ist, dann l�sche Ordnung
            if (!(in_array($_GET['ordnung'], $HEADER))) { unset($_GET['ordnung']); }
    } else {
            if ($_SESSION['html_template'] != 4) $_GET['ordnung'] = $StandardOrdnung;
    }

//    if (is_logged_in()) {   // Zeige "hinzuf�gen", wenn eingeloggt
...
########### Searching
    if (isset($_POST['search']) && ( strlen($_POST['search']) >= 2 )) {
        $search = clean_from_sql($_POST['search']); // Von sch�dlichem Code befreien

        $SuchString_array = explode("," , $search_string[$template]);   // Die Suchspalten aus config ber�cksichtigen

        $query = "SELECT * FROM $tabellen[$template] WHERE ";   // Select Statement f�r den Query vorbereiten

        $query .= array_pop($SuchString_array) . " LIKE '%$search%'";   // Das letzte Element zur Suche hinzuf�gen. Wenns das einzige war, dann ist das Array leer		 

        foreach ($SuchString_array as $SuchString) {    // F�r jede weitere Spalte, in der gesucht wird, verl�ngert sich die SQL Anweisung
