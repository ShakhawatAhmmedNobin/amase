...
redirect('http://newbase.amase-master.net/transcript/'.$id);
        ...
        if (mysql_num_rows($result_grades) != 0 && $Mcourse != "Zusaetzliche Leistung") {
            ...
            while ($row = mysql_fetch_assoc($result_grades)) {   // Kurse aus Modul extrahieren
                ...
                if (($row['modul'] != "Language") && ($row['status'] == "grade")
                   && $row['modul'] != "Voluntary Courses") { // Noten ohne Sprachenschein for the Average Grade
...
In 1st and 2nd semester: min-7 & max-9; In 3rd semester: min-3 & max-5";
            ...
            if (($lastmod != "Language")) {
                $pdf->Cell(25, 5, number_format($sum_ects_and_passed, 1), 1, 0, 'R');
            } else {
...
###### ECTS CREDIT POINTS SUMS  
    if ($pdf->GetY() > 230) $pdf->AddPage();
    $pdf->SetFont('Arial', 'B', $coursenameFont);
    $pdf->Cell(150, 5, "Total ECTS credit points from noted courses UdS", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_benotete, 1) . $ects_sum_array["benotete"], 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from unnoted courses UdS", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_nonote, 1), 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from Zusaetzliche Leistung UdS", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_zus_leist, 1), 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from all courses UdS", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_benotete + $ects_sum_nonote + $ects_sum_zus_leist, 1) . $ects_sum_array["total"], 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from all courses and master project " . $student_from_db['university1'] . " & " . $student_from_db['university2'], 1, 0, 'L'); // Zellraum auff�llen
...
###### ECTS CREDIT POINTS SUMS  
    $pdf->SetFont('Arial', 'B', $coursenameFont);
    $pdf->Cell(150, 5, "Total ECTS credit points from noted courses", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_benotete, 1) . $ects_sum_array["benotete"], 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from unnoted courses", 1, 0, 'L'); // Zellraum auff�llen
    $pdf->Cell(25, 5, number_format($ects_sum_nonote, 1), 1, 0, 'R');
    $pdf->Cell(25, 5, "---", 1, 0, 'C');
    $pdf->Ln();
    $pdf->Cell(150, 5, "Total ECTS credit points from all courses", 1, 0, 'L'); // Zellraum auff�llen
