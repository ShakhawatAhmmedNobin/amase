...
require('zeugnis.common.inc.php');
...
$studentID = $id = guard_login();
$student = getStudentData($db_link, $datenbank, $studentID);
$student = updateStudentData($student);
$result2 = queryGrades($studentID); //TODO: rename result2
###### QUERY MASTER THESIS
$result3 = queryMaster($studentID);
...
{
    $pdf->setSourceFile("./pdf/zeugnis_blanco_umstruk_new.pdf");
} else {
    $pdf->setSourceFile("./pdf/zeugnis_blanco_umstruk.pdf");
}
...
$pdf->SetFont('Univers57cn', '', 12);
$pdf->MultiCell(206.5, 5, utf8_decode("Datum der letzten Prüfungsleistung: ") . $latest_date . utf8_decode("\n\nSaarbrücken, den ") . date("d.m.Y"), 0, 'C');
...
Im 1. und 2. Semester: min-7 & max-9; Im 3. Semester: min-3 & max-5";
            ...
            while ($row = mysql_fetch_assoc($result_grades)) {  // Kurse aus Modul extrahieren
                ...
                if (($row['modul'] != "Language") && ($row['status'] == "grade")  && ($Mcourse != "Voluntary Courses")) {
                    ...
                    if (isValidGrade("UdS", $row['local_grade'])) {
                        if ($row['modul'] != "Zusaetzliche Leistung") {
                        ...
                        echo "2invalid grade: " . $row['local_grade'] . " - university: " . $row['university'] . "<br>";
                    }
    ...
    foreach ($parse_modul_new as $Mcourse) {
        ...
        while ($row = mysql_fetch_assoc($result_partner_uni)) {
            ...
            if (($Mcourse != "Voluntary Courses")
                && ($Mcourse != "Zusaetzliche Leistung")) {
                if (($Mcourse != "Language") && ($row['status'] == "grade")) {
                    $ects_sum_ohne_sprache += $row['credits'];
                    ...
                    if (isValidGrade("UdS", $row['local_grade'])) {
                        ...
                        echo "3invalid grade: " . $row['local_grade'] . " - university: " . $row['university'] . "<br>";
                    }
                }
    ...
    if (mysql_num_rows($result_master_thesis_partner_uni) != 0) {
        ...
        $pdf->SetXY($x, $y);
        if ($row['examiner_grade1'] == "passed" && $row['examiner_grade1'] == "passed") {
            $masterEndnote = "bestanden";
        } else {
            $masterEndnote = substr((((float)$master['examiner_grade1'] + (float)$master['examiner_grade2']) / 2), 0, 3);
        }
            ...
            while ($row = mysql_fetch_assoc($result_grades)) {  // Kurse aus Modul extrahieren
                ...
                if (($row['modul'] != "Language") && ($row['status'] == "grade")) {
                    ...
                    if (isValidGrade("UdS", $row['local_grade'])) {
                        ...
                        echo "1invalid grade: " . $row['local_grade'] . " - university: " . $row['university'] . "<br>";
                    }
                } else if ($row['modul'] == "Language") {
                    $ects_sum_languages += $row['credits'];
...
###### MASTER THESIS   
    if (mysql_num_rows($result3) != 0) {
        $pdf->rand();
        $pdf->SetFont('Univers57cn', 'b', $courseSize);
        $pdf->Cell(190, 7, "Masterarbeit - UdS", 1, 0, 'L');
        $pdf->SetFont('Univers57cn', '', $gradeSize);
        $pdf->Ln();
        $pdf->rand();
...
###### MASTER THESIS   
    if (mysql_num_rows($result3) != 0) {
        $pdf->rand();
        $pdf->SetFont('Univers57cn', 'b', $courseSize);
        $pdf->Cell(190, 8, "Masterarbeit", 1, 0, 'L');
        $pdf->SetFont('Univers57cn', '', $gradeSize);
        $pdf->Ln();
        $pdf->rand();
...
###### 2 SENTENCES
    ...
    $pdf->Cell(210, 5, "Die weiteren Pr�fungsergebnisse sind im Zeugnis der Partner-Universit�t aufgelistet.", 0, 0, 'C');
###### 2 SENTENCES
}
$pdf->Output();
?>
